...
export default function CaseDetail() {
  ...
  if (err && !meta) return (
    <div className="space-y-4">
      <button onClick={() => navigate('/cases')} className="gov-ghost"><ArrowLeft size={14} /> Back to Cases</button>
      <p className="text-sm text-gov-red bg-red-50 border border-red-200 rounded-lg px-4 py-3">{err}</p>
    </div>
  )
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <button onClick={() => navigate('/cases')} className="gov-ghost">
      ...
      <div className="gov-card p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            {meta?.description && <p className="text-xs text-gov-muted mt-1">{meta.description}</p>}
            <p className="text-[11px] text-gov-faint font-mono mt-1.5 flex items-center gap-2">
              <Calendar size={11} />
          ...
          <span className={`text-[11px] px-2.5 py-1 rounded-full border font-mono ${
            status === 'completed' ? 'text-green-700 bg-green-50 border-green-200'
            : status === 'processing' ? 'text-yellow-700 bg-yellow-50 border-yellow-200'
            : 'text-gov-muted bg-gov-wash border-gov-border'
          }`}>
            {status}
          </span>
        </div>
        <div className="flex gap-2 mt-4 flex-wrap">
          <button onClick={openGraph} className="gov-btn">
            <Network size={14} /> Open case graph
          </button>
          {canWrite && (
          <button onClick={rerun} disabled={busy} className="gov-ghost border border-gov-border bg-white disabled:opacity-50">
            <Play size={14} /> {busy ? 'Processing…' : 'Re-run analysis'}
          </button>
          )}
          {canWrite && (!confirmDel ? (
            <button onClick={() => setConfirmDel(true)} className="gov-ghost border border-gov-border bg-white text-gov-red">
              <Trash2 size={14} /> Delete
            </button>
          ) : (
            <button onClick={remove} disabled={busy} className="gov-ghost border border-red-300 bg-red-50 text-gov-red disabled:opacity-50">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="gov-stat">
            <span className="text-xs text-gov-muted">{s.label}</span>
            <p className="text-2xl font-bold text-gov-ink">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 items-start">
        ...
        <div className="gov-card p-4">
          <h2 className="text-sm font-semibold text-gov-ink mb-3 flex items-center gap-2">
            <FileText size={14} className="text-gov-navy" /> Evidence files ({files.length})
          </h2>
          {files.length === 0 ? (
            <p className="text-xs text-gov-muted">No files yet — upload evidence from this page's case card.</p>
          ) : (
            <div className="space-y-2">
              {files.map(f => {
                const ms = mapStatus[f.original]
                return (
                  <div key={f.original} className="bg-gov-wash border border-gov-border rounded-lg p-3">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs text-gov-ink font-mono truncate">{f.original}</p>
                      {ms && (
                        ms.ok
                          ? <span className="flex items-center gap-1 text-[10px] text-gov-igreen flex-shrink-0"><CheckCircle size={11} /> mapped</span>
                          : <span className="flex items-center gap-1 text-[10px] text-yellow-700 flex-shrink-0"><AlertTriangle size={11} /> review{ms.missing.length ? `: ${ms.missing.slice(0, 2).join(', ')}` : ''}</span>
                      )}
                    </div>
                    <p className="text-[11px] text-gov-muted mt-1">
                      {f.format?.toUpperCase()} · detected <span className="font-mono text-gov-muted">{f.detected_type}</span>
        ...
        <div className="gov-card p-4">
          <h2 className="text-sm font-semibold text-gov-ink mb-3 flex items-center gap-2">
            <GitBranch size={14} className="text-gov-red" /> Top leads in this case
          </h2>
          {leads.length === 0 ? (
            <p className="text-xs text-gov-muted">No leads yet — run analysis first.</p>
          ) : (
            <div className="space-y-2">
              {leads.map(l => (
                <div key={l.entity_id} className="flex items-center gap-3 p-2.5 rounded-lg bg-gov-wash border border-gov-border">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gov-ink font-medium truncate">{l.label}</p>
                    <p className="text-[10px] text-gov-muted truncate">{(l.reasons || [])[0] || l.role || ''}</p>
                  </div>
                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border flex-shrink-0 ${
                    l.priority === 'HIGH' ? 'text-red-700 border-red-200 bg-red-50'
                    : l.priority === 'MEDIUM' ? 'text-yellow-700 border-yellow-200 bg-yellow-50'
      ...
      <div className="gov-card p-4">
        <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
          <h2 className="text-sm font-semibold text-gov-ink flex items-center gap-2">
            <BookOpen size={14} className="text-gov-navy" />
            Investigation Dossier
            <span className="text-[10px] font-mono text-gov-muted bg-gov-wash px-1.5 py-0.5 rounded">{blocks.length}</span>
          </h2>
          <div className="flex gap-2 flex-wrap">
            {canWrite && (
            <button onClick={() => setNoteOpen(o => !o)} className="gov-ghost border border-gov-border bg-white text-xs py-1.5">
              <Plus size={12} /> Note
            </button>
            )}
            {canWrite && (
            <button onClick={pinSnapshot} disabled={dosBusy} className="gov-ghost border border-gov-border bg-white text-xs py-1.5 disabled:opacity-50">
              <Plus size={12} /> Snapshot
            </button>
            )}
            <button onClick={loadDossier} disabled={dosBusy} className="gov-ghost border border-gov-border bg-white text-xs py-1.5 disabled:opacity-50">
              <RefreshCw size={12} /> Refresh live
            </button>
            <button onClick={exportPdf} disabled={dosBusy || blocks.length === 0} className="gov-btn text-xs py-1.5 disabled:opacity-50">
        ...
        {noteOpen && (
          <div className="bg-gov-wash border border-gov-border rounded-lg p-3 space-y-2 mb-3">
            <input
              className="gov-input !py-1.5" placeholder="Note title (optional)"
              value={noteTitle} onChange={e => setNoteTitle(e.target.value)}
            />
            <textarea
              className="gov-input" rows={3} placeholder="Observation, hypothesis, next step…"
              value={noteText} onChange={e => setNoteText(e.target.value)}
            />
            <div className="flex justify-end">
              <button onClick={addNote} disabled={dosBusy || !noteText.trim()} className="gov-btn text-xs py-1.5 disabled:opacity-50">
                Save note
              </button>
            </div>
          </div>
        ...
        {blocks.length === 0 ? (
          <p className="text-xs text-gov-muted text-center py-6">
            Empty dossier. Pin entities from the case graph, analyses from Why-Connected, or write the first note above.
          </p>
        ) : (
          <div className="space-y-2">
            {blocks.map(b => (
              <div key={b.id} className="bg-white border border-gov-border rounded-lg p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-gov-border text-gov-muted uppercase flex-shrink-0">
                      {b.kind}
                    </span>
                    <p className="text-xs text-gov-ink font-semibold truncate">
                      {b.title || ({ note: 'Analyst note', entity: b.fresh?.label || 'Entity', explainer: 'Connection analysis', stats: 'Case snapshot' } as any)[b.kind]}
                    </p>
                    {b.changed && (
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-yellow-200 bg-yellow-50 text-yellow-700 flex-shrink-0">
                        UPDATED SINCE PINNED
                      </span>
                    )}
                    {b.missing && (
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-red-200 bg-red-50 text-gov-red flex-shrink-0">
                        UNRESOLVED
                      </span>
                    )}
                  </div>
                  {canWrite && (
                  <button onClick={() => unpin(b.id)} className="text-gov-faint hover:text-gov-red flex-shrink-0" title="Remove block">
                    <X size={13} />
                  </button>
                  )}
                </div>
                ...
                {b.kind === 'note' && b.text && (
                  <p className="text-xs text-gov-ink mt-1.5 whitespace-pre-line">{b.text}</p>
                )}
                {b.kind === 'stats' && b.fresh && (
                  <p className="text-xs text-gov-muted font-mono mt-1.5">
                    {b.fresh.node_count} entities · {b.fresh.edge_count} relationships
                  </p>
                )}
                {b.kind === 'entity' && b.fresh && (
                  <div className="flex items-center justify-between gap-2 mt-1.5">
                    <p className="text-xs text-gov-muted truncate">
                      {b.fresh.cell ? `Cell ${b.fresh.cell} · ` : ''}{b.fresh.role || ''}
                      {b.fresh.lead_score != null ? ` · lead ${b.fresh.lead_score}` : ''}
                    </p>
                    {b.entity_id && (
                      <button onClick={() => openEntity(b.entity_id)} className="text-[11px] text-gov-navy hover:underline flex items-center gap-1 flex-shrink-0">
                ...
                {b.kind === 'explainer' && b.fresh && (
                  <div className="mt-1.5">
                    <p className="text-xs text-gov-ink">{b.fresh.relationship_strength} · score {b.fresh.evidence_score}</p>
                    {b.src && b.dst && (
                      <button onClick={() => openPair(b.src, b.dst)} className="text-[11px] text-gov-navy hover:underline flex items-center gap-1 mt-1">
                ...
                {b.created_by && (
                  <p className="text-[10px] text-gov-faint font-mono mt-1.5">
      ...
      <div className="gov-card p-4">
        <div className="flex items-center justify-between gap-2 mb-3">
          <h2 className="text-sm font-semibold text-gov-ink flex items-center gap-2">
            <ActivityIcon size={14} className="text-gov-igreen" />
            Case Activity
            <span className="text-[10px] font-mono text-gov-muted bg-gov-wash px-1.5 py-0.5 rounded">{activity.length}</span>
          </h2>
          <button onClick={loadActivity} className="gov-ghost border border-gov-border bg-white text-xs py-1.5">
        ...
        {activity.length === 0 ? (
          <p className="text-xs text-gov-muted text-center py-4">No activity yet — actions on this case will appear here.</p>
        ) : (
          <div className="space-y-1.5 max-h-80 overflow-y-auto">
            {activity.map((a, i) => (
              <div key={i} className="bg-gov-wash border border-gov-border rounded-lg px-3 py-2 flex items-start gap-2">
                {a.kind === 'comment'
                  ? <MessageSquare size={12} className="text-gov-navy mt-0.5 flex-shrink-0" />
                  : <span className="w-2 h-2 rounded-full bg-gov-faint mt-1.5 flex-shrink-0" />}
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-gov-ink">
                    <span className="font-mono text-gov-muted">{a.actor || 'system'}</span>
                    {' · '}{a.summary}
                  </p>
                  {a.text && <p className="text-xs text-gov-muted mt-0.5 whitespace-pre-line">{a.text}</p>}
                  <p className="text-[10px] text-gov-faint font-mono mt-0.5">
